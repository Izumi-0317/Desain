...
namespace {
	...
	constexpr float MOVE_SPEED = 0.04f;
}
...
void Player::StateIdle(){
	...
	//攻撃
	if (PUSH(CInput::eMouseL)) m_state = SHit;
...
void Player::StateHave(){
	CVector3D key_dir(0, 0, 0);
	if (!PUSH(CInput::eMouseL) || HOLD(CInput::eMouseL)) {
		//移動
		if (HOLD(CInput::eUp)) key_dir.z = 1;
		if (HOLD(CInput::eDown)) key_dir.z = -1;
		if (HOLD(CInput::eLeft)) key_dir.x = 1;
		...
		if (Camera* f = dynamic_cast<Camera*>(Base::FindObject(eCamera))) {
			if (key_dir.LengthSq() > 0.05f) {
				...
				m_model.ChangeAnimation(Aiming);
			}
		}
	}
	//攻撃
	...
	if (PUSH(CInput::eButton3)) m_state = SAiming;

	if (PUSH(CInput::eNum3)) m_state = SIdle;
}

void Player::StateAiming(){
	if (!Base::FindObject(eScope)) Base::Add(new Scope());
	CVector3D key_dir(0, 0, 0);
	if (!HOLD(CInput::eMouseL) || PUSH(CInput::eMouseL)) {
		//移動
		if (HOLD(CInput::eUp)) key_dir.z = 1;
		if (HOLD(CInput::eDown)) key_dir.z = -1;
		if (HOLD(CInput::eLeft)) key_dir.x = 1;
	...
	if (PUSH(CInput::eButton3)) m_state = SHave;
}
